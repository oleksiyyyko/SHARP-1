...
namespace webAPI.Controllers
{
    ...
    public class AllPlayersController : ControllerBase
    {
        ...
        public AllPlayersController(ApplicationDbContext db)
        {
            _db = db;
        }
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public ActionResult<IEnumerable<Player>> GetModels()
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public ActionResult<Player> Update(int id,[FromBody] Player ModelsDTO)
        {
            if (id <= 0)
            {
                return BadRequest();
            }
            var updatedModel = _db.Players.FirstOrDefault(u => u.Id == id);
            if (updatedModel==null)
            {
                return NotFound();
            }
            updatedModel.Name = ModelsDTO.Name  ;
            _db.SaveChanges();
